...
# def filter_points_by_angle(points, angles, deviation_ranges1=(-80, -100), deviation_ranges3=(80, 100), deviation_ranges2=(-10, 10)):
#     # Define a function that checks if the angle conditions are met
#     def is_within_deviation_range(index):
#         angle = angles[index]
#         prev_angle = angles[index - 1] if index > 0 else None
#         next_angle = angles[index + 1] if index < len(angles) - 1 else None

#         # Define your conditions here, for example:
#         return (prev_angle is not None and prev_angle >= deviation_ranges2[0] and prev_angle <= deviation_ranges2[1] and
#                 (angle >= deviation_ranges1[0] and angle <= deviation_ranges1[1] or angle >= deviation_ranges3[0] and angle <= deviation_ranges3[1]))

#     # Use filter to get indices of points that pass the condition
#     valid_indices = filter(is_within_deviation_range, range(len(points)))

#     # Convert indices to actual points
#     filtered_points = [points[i] for i in valid_indices]

#     # Always include the first and last points
#     if filtered_points and filtered_points[0] != points[0]:
#         filtered_points.insert(0, points[0])
#     if filtered_points and filtered_points[-1] != points[-1]:
