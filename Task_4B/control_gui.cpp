...
void setup()
{
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    html += "</script>";
    html += "<button onclick=\"window.location.href='/control'\">Manual control</button>";
    html += "</body></html>";
  ...
  //   request->send(200, "text/html", "<html><body><h1>Page 2</h1><p>This is Page 2 content.</p></body></html>");
  // });

  server.on("/control", HTTP_GET, [](AsyncWebServerRequest *request) {
    String html = "<html><body>";
    html += "<h1>RC Car Control</h1>";
    html += "<button onclick='sendCommand(\"forward\")'>Forward</button><br>";
    html += "<button onclick='sendCommand(\"left\")'>Left</button>";
    html += "<button onclick='sendCommand(\"right\")'>Right</button><br>";
    html += "<button onclick='sendCommand(\"backward\")'>Backward</button>";
    html += "<script>";
    html += "function sendCommand(command) { fetch('/movem?cmd=' + command); }";
    html += "</script>";
    html += "</body></html>";
    request->send(200, "text/html", html);
  });

  server.on("/movem", HTTP_GET, [](AsyncWebServerRequest *request) {
    command = request->arg("cmd");
    man_signal = !man_signal;
    // if (command == "forward") {
    //   moveForward();
...
//   // Implement forward movement logic for RC car
// }

// void moveBackward() {
//   Serial.println("Moving backward...");
//   // Implement backward movement logic for RC car
// }

// void turnLeft() {
//   Serial.println("Turning left...");
//   // Implement left turn logic for RC car
// }

// void turnRight() {
//   Serial.println("Turning right...");
//   // Implement right turn logic for RC car
// }

void manual_move(String command){  
    if (command == "forward") {
      moveForward();
    } else if (command == "backward") {
      moveBackward();
